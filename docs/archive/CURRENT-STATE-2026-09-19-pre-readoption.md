<!-- Archived 2026-09-19 at framework adoption (ai-project-framework task T-0028). This is the former hand-written docs/CURRENT-STATE.md, preserved intact. That path is now a generated view rendered from project.json and state/releases/; the deployment narrative and capability reconciliation recorded here remain the authority for what they describe until a record supersedes them. The only edits to this copy are mechanical: relative links rebased for the new directory depth and retargeted to the other documents that moved the same day. -->
...
- **The desk is now rehearsed signed in before a deploy** (OC-2 closed, D-CAL-23): `npm run
  ...
  [TASK-TRACKER.md](TASK-TRACKER-2026-09-19-pre-readoption.md). OC-6 is fixed on `main` (`7bb48c9`) and **deployed 2026-09-19 14:14 UTC** in `8683183`.
...
**Remaining scope: optional Batch E (`CAL-P3-001`), the policy-gated Batch J (`CAL-P3-006`, rule D-CAL-6, still needing its own go-ahead) and the deferred optional observation `CAL-P3-007`; Batches F, G, H and I (stacked) await merge and a separate deployment decision.** A/B/C/D implementation and local verification are complete and F, G, H and I are implemented and validated locally on their branches. E2 re-observed every A–D resolution as intact and found no P1; no P2 remains open after Batch H. Hosted and hardware gates still apply. See [TASK-TRACKER.md](TASK-TRACKER-2026-09-19-pre-readoption.md) for status and [COVERAGE.md](../COVERAGE.md) for tested versus partial surfaces.
