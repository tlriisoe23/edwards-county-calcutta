...
**Batch K (2026-09-16, `claude/c2-tv-popup-compact-console`, implemented locally, not merged):** owner-commissioned C2 work (dual-screen TV operation and a compact Auction Night operator layout) — "Launch TV Display" now opens a real popup window sized to the current screen instead of a same-window tab, so the operator console stays independently reachable while the TV is fullscreen on a second display; the operator console now derives a compact layout from `LIVE`/`PAUSED` (with a persistent manual override) that collapses the masthead/heading/event-toolbar and condenses the C1 PREPARE/RUN AUCTION/AFTER AUCTION/TOOLS nav into an inline collapsible section, addressing the deferred `CAL-P3-007`/`D-CAL-5` without a sticky panel. TypeScript, build, lint (48/42 — unchanged from baseline) and both suites (58/58, 72/72) pass on a freshly migrated local store. **No browser-rendered verification was possible in this environment** (no reachable Chrome extension or matching desktop); actual above-the-fold geometry, popup-window behavior and keyboard/focus have not been observed, only reviewed in code — see [BATCH-K.md](BATCH-K.md). Not deployed; the live container is unchanged.
