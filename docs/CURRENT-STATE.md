...
- **S1 Purposeful Themes + Advanced Settings:** accepted by the human and **released**. Committed as `494929b` ("Implement S1 themes, Advanced settings and proportional TV typography"); `task/s1-themes-advanced` fast-forwarded to the same commit, now identical to `main`. `ecgc-calcutta-app-1` was rebuilt (image created 2026-09-16T22:53:28-05:00, ~2 minutes after the commit) and is running healthy. A read-only check in this session (`docker compose ps`, container logs, `GET /api/public`, `GET /`) confirmed the container serves the current production event without error; this only exercised the public read path, not the operator-side theme selector/Advanced disclosure or any authenticated production action. Themes/Advanced, true TV scaling, content containment and navigation contrast are all part of this commit; see [S1.md](S1.md) for scope and measured evidence. Existing lint debt is unchanged; physical hardware and a production operator-side check remain unverified.
- **C2 Auction Night / dual-screen operation:** expected next, not implemented by S1. The [canonical tracker](TASK-TRACKER.md#c2-requirements-recorded-during-s1-recovery--not-implemented) now records secondary-display fullscreen/focus, window placement/manual fallback and compact 1366×768 LIVE/PAUSED controls while preserving C1 navigation.
