...
| CAL-P3-007 | P3 USABILITY | Hammer / bid entry below the fold on first paint at 1280×720 / 1024×768 | DEFERRED (Batch I, D-CAL-5) — a sticky bid panel cannot keep team and current bid visible: even with a compacted panel and block it still covers 76 / 65 px of the bid at 1280×720 / 1024×768 ([sticky-experiment.json](batch-i-evidence/sticky-experiment.json)); meeting the proviso needs ~260 px out of the operator header stack, not a small layout change. **Addressed architecturally by Batch K** (C2, `claude/c2-tv-popup-compact-console`): a state-driven compact console collapses the masthead/heading/event-toolbar and condenses the C1 nav into a collapsible section instead of a sticky panel — see [BATCH-K.md](BATCH-K.md). Statically validated only (tsc/lint/build/58-58/72-72); the actual above-the-fold geometry has **not** been re-measured in a browser, so this is IMPLEMENTED (local, unmeasured), not yet RESOLVED | I (deferred) → K (implemented, unmeasured) |
...
### Batch K — TV popup window and state-driven compact console (C2, implemented locally, unmerged)

- **Status:** Implemented on `claude/c2-tv-popup-compact-console` (branched from `main` at `f4a7e77`) per the owner-approved C2 blueprint (dual-screen TV operation, compact Auction Night operator UI). Not an audit-finding batch and not part of the F–J numbering; recorded here for tracker continuity because it resolves the deferred `CAL-P3-007`/`D-CAL-5` architecturally. TypeScript, `npm run build`, `npm run lint` (48/42, unchanged from baseline), acceptance (58/58) and refinement (72/72) all pass on a freshly migrated local store. **Browser-rendered verification (actual fold geometry, popup window behavior, keyboard/focus, axe) was not possible in this environment and remains UNVERIFIED** — see [BATCH-K.md](BATCH-K.md) for the full breakdown. Not merged, not deployed.
- **Exact IDs:** none new; addresses `CAL-P3-007` without reopening `D-CAL-5`'s decision text.
- **Objective:** Replace the same-window TV tab (`target="_blank"`) with a real popup window so the operator's console stays independently reachable while the TV is fullscreen on a second display; replace the rejected sticky-bid-panel approach to `CAL-P3-007` with a state-driven (LIVE/PAUSED, persistently overridable) compact console that collapses chrome around the block instead of over it.
- **Expected areas:** `lib/sharing.ts`, `app/operator.tsx`, `app/sharing.tsx`, `app/refinements.css`.
- **Risk:** Low–medium — touches shared header/nav CSS also used by C1's nav; no schema/API/business-rule change.
- **Acceptance:** Full acceptance criteria and the outstanding browser-measurement gap are in [BATCH-K.md](BATCH-K.md); do not mark `CAL-P3-007` RESOLVED until the above-the-fold geometry is actually re-measured (the same kind of check `sticky-experiment.json` used for the rejected approach).
- **Order:** independent of F–J; requires its own merge/deploy decision like every other batch.
