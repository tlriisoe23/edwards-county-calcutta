...
2026-09-15 UTC · A/B/C/D approved and verified locally (`4dc2900`, `3d00923`, `4da7b9b`, `a76e57f`) · **F, G, H and I approved and implemented locally on stacked branches ending in `claude/cal-i-data-entry`** (not merged, not deployed; CAL-P3-007 deferred) · optional E awaits approval · no production deployment authorized.
...
| CAL-P2-007 | P2 ACCESSIBILITY | Lot numbers 3.27:1, sale buyer line 4.01:1, payout % 4.01:1, inactive operator tabs 3.70:1, eyebrow 4.27:1 | IMPLEMENTED (local) — Batch H on `claude/cal-h-contrast-tabs`, [BATCH-H.md](BATCH-H.md), [evidence](batch-h-evidence/batch-h.json): now 5.34 / 4.88 / 5.70 / 5.27 / 5.27, axe 0 contrast nodes on public, TV, console, Settlement, Exports; not merged, not deployed | H |
| CAL-P3-002 | P3 ACCESSIBILITY | Flight and settlement filter tabs reference non-existent panels | IMPLEMENTED (local) — Batch H on `claude/cal-h-contrast-tabs`, [BATCH-H.md](BATCH-H.md): real `TabsContent` panels, arrow keys and Tab order unchanged, axe `aria-valid-attr-value` clean; not merged, not deployed | H |
| CAL-P3-003 | P3 USABILITY | Cleared minimum bid / deduction saves 0 with “Saved” | IMPLEMENTED (local) — Batch I on `claude/cal-i-data-entry`, [BATCH-I.md](BATCH-I.md), [evidence](batch-i-evidence/batch-i.json): blank or < $1.00 minimum blocked client- and server-side with “Enter a minimum starting bid of at least $1.00”; Percent/Fixed deduction must be > 0, *None* is the only no-cut; not merged, not deployed | I |
| CAL-P3-004 | P3 USABILITY | Import preview blocks import without marking invalid rows | IMPLEMENTED (local) — Batch I on `claude/cal-i-data-entry`, [BATCH-I.md](BATCH-I.md), [evidence](batch-i-evidence/import-preview-marked-1280.png): per-row “flight not found / name required / index must be a number”, button “Import N teams · M rows need attention”, import still atomic; not merged, not deployed | I |
| CAL-P3-005 | P3 USABILITY | Access tab accepts owner email; duplicate grants report “Saved” | IMPLEMENTED (local) — Batch I on `claude/cal-i-data-entry`, [BATCH-I.md](BATCH-I.md), [evidence](batch-i-evidence/checks.json): owner email → “already an owner”, duplicate → “already has access”, neither writes a row or audit entry; owners listed read-only above operators; not merged, not deployed | I |
