...
| WC-9 | ~~**The public board and the TV call the pop "Index".**~~ **Done 2026-09-19** (D-CAL-32) on `claude/cal-pop-label`: `handicapLabel` is an event setting — *Index*, *Pop* or *Handicap* — used on the block, the cards, the roster and the import preview; an event created from the leaderboard says *Pop*. Acceptance 68 (round-trip through event settings), refinement 72; tsc; eslint 76 (one fewer than the baseline). The import suite's *Pop* check and the console rehearsal follow. |
