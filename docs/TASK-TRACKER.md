...
| OC-9 | **A React hydration error on the desk, in production.** Loading `/admin` signed in on `8683183` logs `Minified React error #418` (server and client rendered different HTML) once, then the desk works. The likeliest cause is the compact-console override read from `localStorage` in a `useState` initialiser (`app/operator.tsx`, `compactOverride`), which the server cannot know; the theme picker's saved value is the other candidate. Reproduce with a dev build to get the unminified message, then move the read into an effect. Seen 2026-09-19 in the owner's Chrome while creating the test event. |
