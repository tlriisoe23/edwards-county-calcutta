...
import { Gavel, Plus, Undo2, Pause, Play, ArrowUp, ArrowDown, GripVertical, ExternalLink, Search, Download, Users, Monitor, ShieldCheck, Pencil, MoreHorizontal, LogOut, Circle, CheckCircle2, AlertCircle, ChevronDown, ChevronUp, Wrench, KeyRound } from 'lucide-react';
...
import { describeUndo } from '@/lib/audit';
...
{e?.demo === 1 && <DropdownMenuItem disabled={busy || offline} onSelect={() => { setReset(''); ask('Reset demo data?', 'This clears all records in the current demonstration event. Other events are kept.', 'reset_demo', {}, { reset: true }); }}>Reset demo data<small>Clears this demonstration event’s teams, buyers and sales</small></DropdownMenuItem>}
 ...
 {saleDraft && data && <SoldDialog key={saleDraft.teamId} sale={saleDraft} setSale={setSaleDraft} data={data} act={act} busy={busy}/>}
 ...
 <AlertDialog open={!!confirm} onOpenChange={open => { if (!open && !busy)
        setConfirm(null); }}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{confirm?.title}</AlertDialogTitle><AlertDialogDescription>{confirm?.description}</AlertDialogDescription></AlertDialogHeader>{confirm?.reset && <Field label="Type RESET DEMO DATA to continue"><Input value={reset} onChange={ev => setReset(ev.target.value)}/></Field>}<AlertDialogFooter><AlertDialogCancel disabled={busy}>Cancel</AlertDialogCancel><AlertDialogAction disabled={busy || confirm?.reset && reset !== 'RESET DEMO DATA'} onClick={async (ev) => { ev.preventDefault(); const c = confirm; const result = c && await act(c.action, c.reset ? { confirmation: reset } : c.payload, { revision: c.revision }); if (result) { c.onComplete?.(result);
        setConfirm(null);
        if (c.sale) {
            setBid('');
            setBuyerId(null);
