...
import { money, parsePaste, tournamentDates, type Row } from '@/lib/model';
export default function Editors({ modal, setModal, data, busy, act, setBuyerId }: any) {
    ...
    async function importFromLeaderboard(slug = source) {
        ...
        try {
            ...
            const rows = (result.rows as Row[]).map((r, i) => ({
                sourceId: r.id ?? null, name: r.name, players: r.players, flightText: r.flight,
    ...
    return <Dialog open={!!modal} onOpenChange={open => { if (!open && !busy)
        setModal(null); }}><DialogContent className={modal?.type === 'import' ? 'wide-dialog' : ''}><DialogHeader><DialogTitle>{({ team: modal?.id ? 'Edit team' : 'Add a team', buyer: modal?.id ? 'Edit buyer' : 'Add a buyer', event: 'Create an event', leaderboard: 'Create an event from the leaderboard', import: 'Import teams', sale: 'Correct sale', buyback: 'Record team buyback', flight: 'Flight details' } as any)[modal?.type || '']}</DialogTitle><DialogDescription>{modal?.type === 'leaderboard' ? 'Reads the tournament straight off the leaderboard: its name, course and dates, its flights, and every team with its pop. Nothing to export — review it, then create.' : modal?.type === 'import' ? 'Read the flighted field straight from the leaderboard, or paste a spreadsheet or CSV. Every row is shown for review before anything is saved.' : modal?.type === 'buyback' ? 'Buyback consideration is arranged directly with the purchaser and never increases the auction pool.' : 'Changes are saved to the event and recorded in its audit trail.'}</DialogDescription></DialogHeader>
 ...
 {modal?.type === 'leaderboard' && (() => { const board = modal.board as Row, lb = board.event as Row, rows = board.rows as Row[], names = board.flights as string[], sources = (board.events || []) as Row[];
        return <div className="dialog-form leaderboard-preview">{sources.length > 1 && <Field label="Tournament on the leaderboard"><select aria-label="Tournament on the leaderboard" value={lb.slug} disabled={busy} onChange={async ev => { const next = await act('leaderboard_field', { slug: ev.target.value }, { quiet: true }); if (next) modalField('board', next); }}>{sources.map((ev: Row) => <option key={ev.slug} value={ev.slug}>{ev.name}{ev.status ? ' · ' + ev.status : ''}</option>)}</select></Field>}
            <dl className="help-definitions"><dt>Tournament</dt><dd>{lb.name}</dd><dt>Course</dt><dd>{lb.course || '—'}</dd><dt>Dates</dt><dd>{tournamentDates(lb.start, lb.end) || '—'}</dd><dt>Flights</dt><dd>{names.length ? names.map(f => f + ' (' + rows.filter(r => r.flight === f).length + ')').join(' · ') : 'none yet'}</dd><dt>Teams</dt><dd>{rows.length ? rows.length + ', each with its pop' : 'none yet'}</dd></dl>
            {board.note && <p className="notice">{board.note} You can still create the event now, and import the field from Teams & flights once the flights are drawn.</p>}
            <Field label="Auction date / time (optional)"><Input type="datetime-local" value={modal.auctionAt} onChange={ev => modalField('auctionAt', ev.target.value)}/></Field>
            <p className="fine">Money rules — minimum bid, increments, house share and payouts — are copied from your most recent event; change them in Event & rules.</p>
            <Button disabled={busy} onClick={async () => { const result = await act('event_from_leaderboard', { slug: lb.slug, auctionAt: modal.auctionAt || undefined }, { quiet: true }); if (result) { setModal(null); toast.success(result.duplicate ? lb.name + ' was already created.' : lb.name + ' created — ' + result.created.flights + ' flight' + (result.created.flights === 1 ? '' : 's') + ', ' + result.created.teams + ' team' + (result.created.teams === 1 ? '' : 's') + (result.rulesFrom ? ' · money rules from ' + result.rulesFrom : '')); } }}>Create event from the leaderboard</Button></div>; })()} {modal?.type === 'event' && <form className="dialog-form" onSubmit={async (ev) => { ev.preventDefault(); const { type, ...v } = modal; if (await act('create_event', v))
