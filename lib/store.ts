...
export function freshEvent(input: Row, actor: string, demo: boolean, requestId: string, audit: { action?: string; after?: Row } = {}) {
    ...
    return { id, event, commands: [insert("events", event), insert("auction_state", { eventId: id, bid: 0, paused: 0 }), insert("audit", { id: requestId, eventId: id, actor, action: audit.action ?? (demo ? "load_demo" : "create_event"), after: JSON.stringify(audit.after ?? input), createdAt: now })] };
}
