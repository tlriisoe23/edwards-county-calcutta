...
import { compute, defaultSettings, type Row } from "./model";
...
export async function read(eventId?: string): Promise<Row | null> {
    ...
    if (!rows[0][0])
        return null;
    const event = { ...rows[0][0], settings: JSON.parse(rows[0][0].settings) };
...
export function publicView(d: Row, light = false) {
    ...
    const state = { teamId: d.state?.teamId, bid: s.showBid ? d.state?.bid : undefined, buyer: s.showBidder ? d.buyers.find((b: Row) => b.id === d.state?.buyerId)?.name : undefined, team: current ? cleanTeam(current) : null, paused: d.state?.paused };
    if (light)
